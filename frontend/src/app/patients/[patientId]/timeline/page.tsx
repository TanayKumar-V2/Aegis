...
import { ArrowLeft, CalendarDays, ClipboardList, Download, FilePlus2, Loader2, Pill, ShieldAlert, Stethoscope } from "lucide-react";
...
export default function TimelinePage() {
  ...
  async function exportPatientRecord() {
    setIsExporting(true);
    try {
      const response = await api.get<Blob>(`/patients/${patientId}/export-pdf`, { responseType: "blob" });
      const contentDisposition = response.headers["content-disposition"] as string | undefined;
      const filename = contentDisposition?.match(/filename="?([^";]+)"?/)?.[1] || `aegis-export-${patientId}.pdf`;
      ...
      toast.success("Patient record exported as PDF.");
    } catch (error) {
      toast.error(getApiErrorMessage(error, "We could not export this patient record."));
    } finally {
  ...
  return <><DashboardHeader /><main className="mx-auto w-full max-w-5xl px-6 py-10 lg:px-10"><div className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><Link href="/dashboard" className="mb-5 inline-flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-primary"><ArrowLeft className="size-4" />Back to workspace</Link><p className="text-sm font-semibold uppercase tracking-[0.16em] text-primary">Shared patient timeline</p><h1 className="mt-3 text-4xl font-semibold tracking-tight text-slate-900">Care record</h1><div className="mt-3 flex flex-wrap items-center gap-3"><p className="flex max-w-full items-center gap-2 break-all text-sm text-slate-500"><span className="size-2 shrink-0 rounded-full bg-emerald-500" />Patient ID {patientId}</p>{user.role === "caregiver" && <Badge variant="secondary">Read-only view</Badge>}</div></div><div className="flex flex-wrap items-center gap-3"><Button variant="outline" className="rounded-xl" onClick={exportPatientRecord} disabled={isExporting}>{isExporting ? <Loader2 className="mr-2 size-4 animate-spin" /> : <Download className="mr-2 size-4" />}{isExporting ? "Preparing PDF…" : "Export PDF"}</Button>{user.role === "doctor" && <NewEntryDialog patientId={patientId} onCreated={(entry) => setEntries((current) => [entry, ...current])} />}</div></div><div className="space-y-6"><ActiveAlerts flags={flags} canResolve={user.role === "doctor"} onResolve={setResolveFlag} /><Card className="border-white/80 bg-white/75 shadow-sm"><CardContent className="p-5 sm:p-7"><div className="mb-8 flex items-center justify-between"><div><h2 className="text-lg font-semibold text-slate-800">Activity</h2><p className="mt-1 text-sm text-slate-500">Newest entries first</p></div>{unresolvedCount > 0 && <Badge variant="destructive">{unresolvedCount} active alert{unresolvedCount === 1 ? "" : "s"}</Badge>}</div>{entries.length === 0 ? <EmptyState icon={ClipboardList} message="No records yet — entries from your care team will appear here" /> : <div className="space-y-5">{entries.map((entry) => <article key={entry.id} className={`relative border-l-4 pl-5 ${entryColors[entry.entry_type]}`}><div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-start"><div><div className="flex flex-wrap items-center gap-2"><Badge variant="outline" className="bg-card text-card-foreground">{entryLabels[entry.entry_type]}</Badge><Badge variant="secondary">{entry.specialty_tag}</Badge></div><h3 className="mt-3 text-lg font-semibold text-slate-800">{entry.title}</h3></div><time className="flex items-center gap-1.5 text-xs text-slate-400"><CalendarDays className="size-3.5" />{formatDate(entry.created_at)}</time></div><p className="mt-3 whitespace-pre-wrap text-sm leading-7 text-slate-600">{entry.content}</p><p className="mt-4 flex items-center gap-2 text-xs text-slate-400"><Stethoscope className="size-3.5" />Authored by {entry.author_id.slice(0, 8)}…</p></article>)}</div>}</CardContent></Card></div></main><ResolveDialog flag={resolveFlag} onClose={() => setResolveFlag(null)} onResolved={onResolved} /></>;
}
